...
def arg_parser(*arguments):
    ...
    parser_mark_in_progress = subparsers.add_parser(
        "prog", help="Mark a task as in-progress"
    )
    parser_mark_in_progress.add_argument(
        "task_id", type=int, help="ID of the task to mark as in-progress"
    )

    # 'done' command
    parser_mark_done = subparsers.add_parser("done", help="Mark a task as done")
    parser_mark_done.add_argument(
        "task_id", type=int, help="ID of the task to mark as done"
    )
